<template>
  <div>
    ...
    <div class="a-header">
      <a-col class="gutter-row" :span="4">
        <div class="gutter-box">
          <label>承揽方式:</label>
          <a-input placeholder="承揽方式" v-model="headerForm.name"/>
        </div>
      </a-col>
      <a-col class="gutter-row" :span="4">
        <div class="gutter-box">
    ...
    <a-modal v-model="modalState" :title="modalTitle" :footer="null">
      ...
      >
        <a-form-model-item label="承揽方式" prop="name">
          <a-input v-model="form.name" placeholder="承揽方式"/>
        </a-form-model-item>
...
<script>
  import {insertEngineeringUndertake,updateEngineeringUndertake,batchDeleteEngineeringUndertake,queryEngineeringUndertakeList,queryEngineeringUndertakePage,deleteEngineeringUndertakeByid} from "@/api/ProjectInformationManagement/ProjectCode/WayContract";
  const columns = [
    { title: '承揽方式',dataIndex: 'name',key: 'name' },
  ...
  const paginationMixins = {
    ...
    methods: {
      ...
      onPaginationChange (pageNumber, pageSize) {
        ...
        this.queryEngineeringUndertakePage();
      },
      // 分页下拉改变
    ...
    methods: {
      ...
      onAdd(){
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            insertEngineeringUndertake(this.form)
              .then(res => {
                if(res.code==2020200){
                  this.queryEngineeringUndertakePage();
      ...
      onUpdate(){
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            updateEngineeringUndertake(Object.assign(this.form,{id:this.id}))
              .then(res => {
                if(res.code==2020200){
                  this.queryEngineeringUndertakePage();
                  this.modalState = false;
  ...
  const tableMixins = {
    ...
    methods: {
      //查询
      queryEngineeringUndertakePage(){
        this.spinning = true;
        queryEngineeringUndertakePage(Object.assign(this.page,this.headerForm))
          .then(res => {
            if(res.code==2020200){
      ...
      onDeleteConfirm (record) {
        deleteEngineeringUndertakeByid({id:record.id})
          .then(res => {
            if(res.code==2020200){
              this.queryEngineeringUndertakePage();
              this.$message.info(res.message);
