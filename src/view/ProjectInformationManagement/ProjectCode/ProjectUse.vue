<template>
  <div>
    ...
    <div class="a-header">
      <a-col class="gutter-row" :span="4">
        <div class="gutter-box">
          <label>工程用途:</label>
          <a-input placeholder="工程用途" v-model="headerForm.name"/>
        </div>
      </a-col>
      <a-col class="gutter-row" :span="4">
        <div class="gutter-box">
    ...
    <a-modal v-model="modalState" :title="modalTitle" :footer="null">
      ...
      >
        <a-form-model-item label="工程用途" prop="name">
          <a-input v-model="form.name" placeholder="工程用途"/>
        </a-form-model-item>
...
<script>
  import {insertEngineeringUse,updateEngineeringUse,batchDeleteEngineeringUse,queryEngineeringUseList,queryEngineeringUsePage,deleteEngineeringUseByid} from "@/api/ProjectInformationManagement/ProjectCode/ProjectUse";
  const columns = [
    { title: '工程用途',dataIndex: 'name',key: 'name' },
    ...
    methods: {
      ...
      onAdd(){
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            insertEngineeringUse(this.form)
              .then(res => {
                if(res.code==2020200){
                  this.queryEngineeringUsePage();
      ...
      onUpdate(){
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            updateEngineeringUse(Object.assign(this.form,{id:this.id}))
              .then(res => {
                if(res.code==2020200){
                  this.queryEngineeringUsePage();
                  this.modalState = false;
  ...
  const tableMixins = {
    ...
    methods: {
      //查询
      queryEngineeringUsePage(){
        this.spinning = true;
        queryEngineeringUsePage(Object.assign(this.page,this.headerForm))
          .then(res => {
            if(res.code==2020200){
      ...
      onDeleteConfirm (record) {
        deleteEngineeringUseByid({id:record.id})
          .then(res => {
            if(res.code==2020200){
              this.queryEngineeringUsePage();
              this.$message.info(res.message);
