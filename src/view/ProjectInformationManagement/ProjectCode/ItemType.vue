<template>
  <div>
    ...
    <div class="a-header">
      <a-col class="gutter-row" :span="4">
        <div class="gutter-box">
          <label>项目类型:</label>
          <a-input placeholder="项目类型" v-model="headerForm.name"/>
        </div>
      </a-col>
      <a-col class="gutter-row" :span="4">
        <div class="gutter-box">
    ...
    <a-modal v-model="modalState" :title="modalTitle" :footer="null">
      ...
      >
        <a-form-model-item label="项目类型" prop="name">
          <a-input v-model="form.name" placeholder="项目类型"/>
        </a-form-model-item>
...
<script>
  import {insertProjectType,updateProjectType,batchDeleteProjectType,queryProjectTypeList,queryProjectTypePage,deleteProjectTypeByid} from "@/api/ProjectInformationManagement/ProjectCode/ItemType";
  const columns = [
    { title: '项目类型',dataIndex: 'name',key: 'name' },
  ...
  const formModeMixins = {
    ...
    methods: {
      ...
      onUpdate(){
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            updateProjectType(Object.assign(this.form,{id:this.id}))
              .then(res => {
                if(res.code==2020200){
                  this.queryProjectTypePage();
                  this.modalState = false;
  ...
  const tableMixins = {
    ...
    methods: {
      ...
      onDeleteConfirm (record) {
        deleteProjectTypeByid({id:record.id})
          .then(res => {
            if(res.code==2020200){
              this.queryProjectTypePage();
              this.$message.info(res.message);
